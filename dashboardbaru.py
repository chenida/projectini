...
daily_expense = get_daily_series(df)
...
# ======================
@st.cache_resource
def train_prophet(daily_expense):
    # 1. Pastikan kita membuat copy agar tidak merusak data asli
    prophet_df = daily_expense.copy()
    ...
    prophet_df = prophet_df.rename(columns={
        ...
        'Pengeluaran': 'y'      # Ganti 'Pengeluaran' dengan kolom yang ingin diprediksi
    })
...
def train_xgboost(_daily):
    feat_df = create_advanced_features(_daily['Pengeluaran'])
...
if not daily_expense.empty and len(daily_expense) > 30:
    with st.spinner("🔄 Training model..."):
        prophet_model = train_prophet(daily_expense)
        xgb_model, feat_df, feature_cols, xgb_mae, xgb_rmse, xgb_mape, y_test_xgb, y_pred_xgb = train_xgboost(daily_expense)
    
    split_idx = int(0.8 * len(daily_expense))
    test_dates = daily_expense.index[split_idx:]
    
    # Prepare data for Prophet evaluation
    prophet_eval_df = daily_expense.copy()
    ...
    prophet_eval_df = prophet_eval_df.rename(columns={'Tanggal': 'ds', 'Pengeluaran': 'y'})
    ...
    prophet_test_true = daily_expense['Pengeluaran'].iloc[split_idx:].values
    ...
    def xgb_future_predict(model, daily, feature_cols, days=30):
        extended = daily['Pengeluaran'].copy()
        predictions = []
    ...
    xgb_future_dates, xgb_future_pred = xgb_future_predict(xgb_model, daily_expense, feature_cols, days=30)
...
# Date filter
date_min = daily_expense.index.min().date() if not daily_expense.empty else datetime(2023,1,1).date()
date_max = daily_expense.index.max().date() if not daily_expense.empty else datetime(2026,5,31).date()
...
# Filter data by date
filtered_expense = daily_expense[
    (daily_expense.index >= pd.Timestamp(start_date)) &
    (daily_expense.index <= pd.Timestamp(end_date))
] if not daily_expense.empty else pd.DataFrame()

# Filter expense data by selected categories
if not expense_df.empty:
    filtered_category_expense = expense_df[expense_df['Kategori'].isin(selected_categories)] if selected_categories else expense_df
    filtered_category_expense = filtered_category_expense[
        (filtered_category_expense['Tanggal'] >= pd.Timestamp(start_date)) &
        (filtered_category_expense['Tanggal'] <= pd.Timestamp(end_date))
    ]
else:
    filtered_category_expense = pd.DataFrame()

# ======================
# HEADER
# ======================
st.title("💰 Dashboard Keuangan GenZ — Model B: Prophet + XGBoost")
st.markdown("**Analisis Pengeluaran per Kategori + Prediksi Pengeluaran**")
st.markdown("---")

# KPI Row
col1, col2, col3, col4, col5 = st.columns(5)

total_expense = df['Total'].sum() if not df.empty else 0

if not daily_expense.empty and (daily_expense.index.max() - daily_expense.index.min()).days > 0:
    avg_monthly_expense = total_expense / ((daily_expense.index.max() - daily_expense.index.min()).days / 30.44)
else:
...
if model_trained:
    if selected_model == "Prophet":
        ...
        avg_pred_expense = float(prophet_30['yhat'].mean())
    elif selected_model == "XGBoost":
        _, xgb_pred_30 = xgb_future_predict(xgb_model, daily_expense, feature_cols, days=forecast_days)
        avg_pred_expense = float(xgb_pred_30.mean())
    else:
        p30 = float(prophet_future_predict(prophet_model, days=forecast_days)['yhat'].mean())
        _, x30 = xgb_future_predict(xgb_model, daily_expense, feature_cols, days=forecast_days)
        avg_pred_expense = (p30 + float(x30.mean())) / 2
else:
    avg_pred_expense = 0
...
with col2:
    st.metric("📊 Rata-rata per Hari", f"Rp{daily_expense['Pengeluaran'].mean()/1e3:.0f}K" if not daily_expense.empty else "Rp0")
with col3:
    st.metric("📅 Rata-rata per Bulan", f"Rp{avg_monthly_expense/1e6:.1f}Jt")
with col4:
    st.metric("🏆 Top Kategori", top_category)
with col5:
    st.metric(f"🔮 Prediksi Pengeluaran ({forecast_days}H)", f"Rp{avg_pred_expense/1e3:,.0f}K")
...
# ---- TAB 1: PREDIKSI ----
with tab1:
    st.subheader(f"Prediksi Pengeluaran {forecast_days} Hari ke Depan — {selected_model}")
    ...
    if model_trained:
        ...
        if selected_model in ["XGBoost", "Ensemble"]:
            xfd, xfp = xgb_future_predict(xgb_model, daily_expense, feature_cols, days=forecast_days)
        
        fig = go.Figure()
        
        # Historical
        mask = (daily_expense.index >= pd.Timestamp(start_date)) & (daily_expense.index <= pd.Timestamp(end_date))
        hist = daily_expense[mask]
        ...
        fig.add_vline(
            x=daily_expense.index[-1], 
            line_dash="dot", 
            line_color="gray"
        )
        
        # Tambahkan teks "Hari Ini" secara manual
        fig.add_annotation(
            x=daily_expense.index[-1],
            y=1,
            yref="paper",
        ...
        fig.update_layout(
            title="Pengeluaran Harian + Prediksi", 
            xaxis_title="Tanggal",
...
with tab3:
    ...
    if model_trained:
        ...
        # Visual comparison
        test_dates_plot = daily_expense.index[split_idx:]
        actual_test = daily_expense['Pengeluaran'].iloc[split_idx:].values
        ...
        fig_ab.update_layout(title="Perbandingan Model pada Data Test",
                             xaxis_title="Tanggal", yaxis_title="Pengeluaran (Rp)", height=400)
...
# ---- TAB 5: RINGKASAN ----
with tab5:
    st.subheader("📋 Ringkasan Pengeluaran GenZ")
    ...
    with col_r1:
        ...
        if not expense_df.empty:
            ...
            st.metric("Kategori Terbanyak", f"{expense_df['Kategori'].mode().iloc[0] if not expense_df.empty else '-'}")
            st.metric("Produk Terbanyak", f"{expense_df['Produk'].mode().iloc[0] if not expense_df.empty else '-'}")
    
    with col_r2:
        st.markdown("### Statistik Harian")
        if not daily_expense.empty:
            st.metric("Hari Pengeluaran Tertinggi", 
                     f"{daily_expense['Pengeluaran'].idxmax().strftime('%d %b %Y') if not daily_expense.empty else '-'}")
            st.metric("Nominal Tertinggi", 
                     f"Rp{daily_expense['Pengeluaran'].max():,.0f}" if not daily_expense.empty else "Rp0")
            st.metric("Hari Pengeluaran Terendah", 
                     f"{daily_expense['Pengeluaran'].idxmin().strftime('%d %b %Y') if not daily_expense.empty else '-'}")
            st.metric("Nominal Terendah", 
                     f"Rp{daily_expense['Pengeluaran'].min():,.0f}" if not daily_expense.empty else "Rp0")
            st.metric("Standar Deviasi", 
                     f"Rp{daily_expense['Pengeluaran'].std():,.0f}" if not daily_expense.empty else "Rp0")
    
    st.markdown("---")
    st.markdown("### 💡 Insight & Rekomendasi Pengeluaran")
    ...
    if not category_total.empty:
        ...
        highest_cat = category_total.iloc[0]['Kategori']
        highest_amount = category_total.iloc[0]['Total_Pengeluaran']
        insights.append(f"💰 **Kategori terbesar:** {highest_cat} (Rp{highest_amount/1e6:.1f}Jt)")
        
        # Recommendation based on top category
        if highest_cat == "Makanan":
            insights.append("🍜 **Rekomendasi:** Coba masak di rumah lebih sering dan kurangi pesan antar.")
        elif highest_cat == "Transportasi":
            insights.append("🚗 **Rekomendasi:** Pertimbangkan transportasi umum atau carpooling untuk menghemat.")
        elif highest_cat == "Hiburan":
            insights.append("🎮 **Rekomendasi:** Batasi pengeluaran hiburan dan cari alternatif gratis.")
        elif highest_cat == "Belanja":
            insights.append("🛍️ **Rekomendasi:** Buat wishlist dan tunggu 3 hari sebelum membeli barang non-esensial.")
    
    # Daily pattern insights
    if not expense_df.empty:
        expense_df['Hari'] = expense_df['Tanggal'].dt.day_name()
        day_avg = expense_df.groupby('Hari')['Total'].mean()
        highest_day = day_avg.idxmax()
        insights.append(f"📅 **Hari dengan pengeluaran tertinggi:** {highest_day}")
        
        if highest_day in ['Saturday', 'Sunday']:
            insights.append("🎉 **Rekomendasi:** Kurangi pengeluaran di akhir pekan dengan mencari aktivitas gratis.")
    
    # Monthly pattern insights
    if not expense_df.empty:
        expense_df['Bulan'] = expense_df['Tanggal'].dt.month
        monthly_avg = expense_df.groupby('Bulan')['Total'].mean()
        highest_month = monthly_avg.idxmax()
        month_names_full = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 
                           'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
        insights.append(f"📆 **Bulan dengan pengeluaran tertinggi:** {month_names_full[highest_month-1]}")
    
    # Future prediction insights
    if model_trained:
        insights.append(f"🔮 **Prediksi pengeluaran {forecast_days} hari ke depan:** Rp{avg_pred_expense/1e3:.0f}K")
        
        # Compare with average
        current_avg = daily_expense['Pengeluaran'].mean()
        if avg_pred_expense > current_avg:
            insights.append(f"⚠️ **Peringatan:** Prediksi pengeluaran {((avg_pred_expense/current_avg)-1)*100:.1f}% di atas rata-rata. Perlu lebih berhemat!")
        else:
            insights.append(f"✅ **Optimis:** Prediksi pengeluaran {((1-avg_pred_expense/current_avg))*100:.1f}% di bawah rata-rata. Pertahankan!")
    ...
    with col_action1:
        st.markdown("**Short-term (Minggu ini):**")
        st.markdown("""
        1. Catat setiap pengeluaran kecil
        2. Batasi maksimal pengeluaran per hari
        3. Hindari pembelian impulsif
        """)
    
    with col_action2:
        st.markdown("**Long-term (Bulan ini):**")
        st.markdown("""
        1. Buat anggaran per kategori
        2. Cari promo/diskon untuk kebutuhan pokok
        3. Evaluasi subscription yang tidak terpakai
        """)